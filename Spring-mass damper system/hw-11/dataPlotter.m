classdef dataPlotter < handle
    ...
    methods
        function self = dataPlotter(p)
           ...
           self.time_history=NaN*ones(1,(p.tend-p.start)/p.t_plot);
           self.r_history=NaN*ones(1,(p.tend-p.start)/p.t_plot);
           self.y_history=NaN*ones(1,(p.tend-p.start)/p.t_plot);
           self.ydot_history=NaN*ones(1,(p.tend-p.start)/p.t_plot);
           self.u_history= NaN*ones(1,(p.tend-p.start)/p.t_plot);
           self.index=1;
